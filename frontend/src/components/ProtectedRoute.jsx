  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gym-black flex items-center justify-center">
        <motion.div
          className="rounded-full h-16 w-16 border-t-4 border-b-4 border-gym-green"
          animate={{ rotate: 360 }}
  ...
  if (!allowedRoles.includes(user?.role)) {
    return (
      <div className="min-h-screen bg-gym-black flex items-center justify-center px-4">
        <motion.div className="bg-red-900/20 border border-red-500 p-8 rounded-lg text-center max-w-lg" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18 }}>
